...
and descriptions of variables are:
 ...
 -------------------------------------------------------------------------------
```{r}
yusample <- read.csv("yu2017sample.csv")
```
## Question 1
...
```{r}
unique_women <- length(unique(yusample$PUBID))
```

There are 1569 unique women and 18282 person-years included in this dataset. 
...
```{r}
yusample$logwage <- log(yusample$wage)

test <- cor(yusample$logwage, yusample$competitiveness)

cor(yusample$logwage, yusample$experience)

```

It is important to use a log because it represents a rate of change rather than an absolute number
...
```{r}
(lm_fe_1 <- lm(logwage ~ numChildren + factor(year) + factor(PUBID), data = yusample))
coef(lm_fe_1)["numChildren"]
```
