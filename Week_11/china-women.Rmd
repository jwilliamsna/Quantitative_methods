...
tea_producing <- subset(chinawomen.1985, subset = teasown > 0)

non_tea <- subset(chinawomen.1985, subset = teasown == 0)
...
```{r}
##mean sex ratio
#est_tea <- mean(tea_producing$sex)

#est_no <- mean(non_tea$sex)

#conf.level <- .05
#se <- sqrt(var(tea_producing$sex) / nrow(tea_producing))
#
#ci <- c(est_tea - qnorm(1 - conf.level /  2) * se,
 #       est_tea + qnorm(1 - conf.level /  2) * se)

ci_fun <- function(df, conf) {
  est <- mean(df$sex)
  
  se <- sqrt(var(df$sex) / nrow(df))
  
  ci <- c(est - qnorm(1 - conf / 2) * se,
          est + qnorm(1 - conf / 2) * se)
  return(ci)
}

ci_tea <- ci_fun(tea_producing, .05)
